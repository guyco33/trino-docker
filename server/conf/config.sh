...
then
    cp /opt/trino/etc/config-coordinator.properties /opt/trino/etc/config.properties
    if [[ ${COORDINATOR_IS_WORKER} == 'true' ]]
    then
        sed -i -e "s|node-scheduler.include-coordinator=false|node-scheduler.include-coordinator=true|g" /opt/trino/etc/config.properties
    fi
else
    cp /opt/trino/etc/config-worker.properties /opt/trino/etc/config.properties
fi

cp /opt/trino/etc/hive.template.properties /opt/trino/etc/catalog/hive.properties
...
#set_s3_accees_key /opt/trino/etc/catalog/hive.properties
set_gcs_credentials /opt/trino/etc/catalog/hive.properties

if [[ -n "${ICEBERG_DIR}" ]]
then
    cp /opt/trino/etc/iceberg.template.properties /opt/trino/etc/catalog/iceberg.properties
    sed -i -e "s|%ICEBERG_DIR%|${ICEBERG_DIR}|g" /opt/trino/etc/catalog/iceberg.properties
#    set_s3_accees_key /opt/trino/etc/catalog/iceberg.properties
else
    rm -f /opt/trino/etc/catalog/iceberg.properties
fi

if [[ -z "${KINESIS_DIR}" ]] && [[ -d "/opt/trino/etc/kinesis" ]]
then
    KINESIS_DIR="/opt/trino/etc/kinesis"
fi
if [[ -n "${KINESIS_DIR}" ]]
then
    cp /opt/trino/etc/kinesis.template.properties /opt/trino/etc/catalog/kinesis.properties
    sed -i -e "s|%KINESIS_DIR%|${KINESIS_DIR}|g" /opt/trino/etc/catalog/kinesis.properties
#    set_s3_accees_key /opt/trino/etc/catalog/kinesis.properties
fi